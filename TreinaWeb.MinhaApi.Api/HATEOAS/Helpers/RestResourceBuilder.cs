    ...
    public class RestResourceBuilder
    {
        public static void BuildResource(object resource, HttpRequestMessage request)
        {
            ...
            if (dtoType.BaseType != typeof(RestResource))
                throw new ArgumentException($"Era esperado um 'RestResource', mas foi enviado um {resource.GetType().FullName}");

            Assembly currentAssembly = Assembly.GetExecutingAssembly();
            IResourceBuilders resourceBuilder
                = (IResourceBuilders)Activator.CreateInstance(currentAssembly.GetType($"TreinaWeb.MinhaApi.Api.HATEOAS.ResourceBuilders.Impl.{dtoType.Name}ResourceBuilder"));
            if (enumerable == null)
            {
